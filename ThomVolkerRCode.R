...
## Center all variables of interest and select only the first 88 rows, other rows are added later on
...
cor_p <- mean(-1 * apply(sim_res,2, function(x) cor(qnorm(ppoints(x)),sort(x))) >
     -1 * apply(obs_res,2, function(x) cor(qnorm(ppoints(x)),sort(x))))
